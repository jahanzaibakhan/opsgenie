...
if [[ ${#ELIGIBLE_APPS[@]} -eq 0 ]]; then
    echo -e "${YELLOW}⚠️ No failed apps with backup data found.${NC}"
elif [[ ${#ELIGIBLE_APPS[@]} -gt 2 ]]; then
    while true; do
        read -rp "$(echo -e "More than 2 failed apps found (${#ELIGIBLE_APPS[@]} apps). Take backup for ${BOLD}all${NC} failed apps now? [Y/N]: ")" BACKUP_ALL </dev/tty
        BACKUP_ALL=$(echo "$BACKUP_ALL" | tr '[:upper:]' '[:lower:]')
